<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="white--text text-center" v-if="language === 'en'">What do you need?</h1>
        <h1 class="white--text text-center" v-if="language === 'ru'">Что Вам нужно?</h1>
      </v-col>
    </v-row>
    <v-row>
    ...
    <v-row class="pa-12 pt-0">
      <v-col cols="12" md="4" class="pa-5" v-for="job in typeJobs" :key="job.id">
        <v-card dark outlined class="main-bg" :class="typeJobSelected === job.id ? 'elevation-10' : ''" v-if="language === 'en'">
          <v-card-title style="word-break: normal">{{job.nameEn}}</v-card-title>
          <!-- <v-card-text v-text="job.shortDescEn"></v-card-text> -->
          <v-card-actions>
            <!-- <v-btn text>Share</v-btn> -->
            <v-btn color="purple" v-if="typeJobSelected !== job.id" text @click="selectJobType(job.id)">Select</v-btn>
            <v-btn color="success" v-else text @click="selectJobType(job.id)">Unselect</v-btn>
            <v-spacer></v-spacer>
            <v-btn icon @click="job.showDesc = !job.showDesc">
              <v-icon>{{ job.showDesc ? 'mdi-chevron-up' : 'mdi-chevron-down' }}</v-icon>
            </v-btn>
          </v-card-actions>
          <v-expand-transition>
            <div v-show="job.showDesc">
              <v-divider></v-divider>
              <v-card-text v-text="job.shortDescEn" class="text-justify"></v-card-text>
            </div>
          </v-expand-transition>
        </v-card>
        <v-card dark outlined class="main-bg" :class="typeJobSelected === job.id ? 'elevation-10' : ''" v-if="language === 'ru'">
          <v-card-title style="word-break: normal">{{job.nameRu}}</v-card-title>
          <!-- <v-card-text v-text="job.shortDescEn"></v-card-text> -->
          <v-card-actions>
            <!-- <v-btn text>Share</v-btn> -->
            <v-btn color="purple" v-if="typeJobSelected !== job.id"  text @click="selectJobType(job.id)">Выбрать</v-btn>
            <v-btn color="success" v-else  text @click="selectJobType(job.id)">Убрать</v-btn>
            <v-spacer></v-spacer>
            <v-btn icon @click="job.showDesc = !job.showDesc">
              <v-icon>{{ job.showDesc ? 'mdi-chevron-up' : 'mdi-chevron-down' }}</v-icon>
            </v-btn>
          ...
          <v-expand-transition>
            <div v-show="job.showDesc">
              <v-divider></v-divider>
              <v-card-text v-text="job.shortDescRu" class="text-justify"></v-card-text>
            </div>
          </v-expand-transition>
        </v-card>
      </v-col>
    ...
    <v-row>
      <v-col class="px-12">
        <h3 class="white--text additional-services" v-if="language === 'en'">Additional requirements</h3>
        <h3 class="white--text additional-services" v-if="language === 'ru'">Дополнительные требования</h3>
      </v-col>
    </v-row>
    <v-row class="pa-12">
      <v-col cols="12" md="4" class="pa-5" v-for="job in typeAdditonalJobs" :key="job.id">
        <v-card dark outlined class="main-bg" :class="job.selected ? 'elevation-10' : ''" v-if="language === 'en'">
          <v-card-title style="word-break: normal">{{job.nameEn}}</v-card-title>
          <v-card-actions>
            <v-btn color="purple" v-if="!job.selected" text @click="job.selected = !job.selected">Select</v-btn>
            <v-btn color="success" v-else text @click="job.selected = !job.selected">Unselect</v-btn>
            <v-spacer></v-spacer>
            <v-btn icon @click="job.showDesc = !job.showDesc">
              <v-icon>{{ job.showDesc ? 'mdi-chevron-up' : 'mdi-chevron-down' }}</v-icon>
            </v-btn>
        ...
        <v-card dark outlined class="main-bg" :class="job.selected ? 'elevation-10' : ''" v-if="language === 'ru'">
          <v-card-title style="word-break: normal">{{job.nameRu}}</v-card-title>
          <!-- <v-card-text v-text="job.shortDescEn"></v-card-text> -->
          <v-card-actions>
            <!-- <v-btn text>Share</v-btn> -->
            <v-btn color="purple" v-if="!job.selected"  text @click="job.selected = !job.selected">Выбрать</v-btn>
            <v-btn color="success" v-else  text @click="job.selected = !job.selected">Убрать</v-btn>
            <v-spacer></v-spacer>
            <v-btn icon @click="job.showDesc = !job.showDesc">
              <v-icon>{{ job.showDesc ? 'mdi-chevron-up' : 'mdi-chevron-down' }}</v-icon>
            </v-btn>
    ...
    <v-row>
      <v-col class="px-12">
        <h3 class="white--text additional-services" v-if="language === 'en'">Additional services</h3>
        <h3 class="white--text additional-services" v-if="language === 'ru'">Дополнительные услуги</h3>
      </v-col>
    </v-row>
    <v-row class="pa-12">
      <v-col cols="12" md="4" class="pa-5" v-for="job in typeAdditonalServices" :key="job.id">
        <v-card dark outlined class="main-bg" :class="job.selected ? 'elevation-10' : ''" v-if="language === 'en'">
          <v-card-title style="word-break: normal">{{job.nameEn}}</v-card-title>
          <v-card-actions>
            <v-btn color="purple" v-if="!job.selected" text @click="job.selected = !job.selected">Select</v-btn>
  ...
  data: () => ({
    ...
    typeJobs: [
      ...
      },
      {
        ...
        shortDescRu: 'Простой сайт по типу одностраничника (Single Page Application)',
        shortDescEn: 'I will develop a simple site like Single Page Application.'
      },
      {
        id: 3,
        showDesc: false,
    ...
    typeAdditonalJobs: [
      ...
      },
      {
        ...
        shortDescRu: 'Отправка E-mail уведомлений с Вашего сайта. Например через кнопку "Заказать обратный звонок", "Купить продукт", "Оставить отзыв"',
        shortDescEn: 'Send e-mail notifications from your website. For example throw button "Make order", "Call me", "Send feedback"'
      },
      {
        id: 3,
        showDesc: false,
        selected: false,
        nameRu: 'Привязать оплату через Yandex кошелек (или другой платежный сервис)',
      ...
      },
      {
        ...
        shortDescRu: 'Помогу реализовать слой авторизации для вашего приложения. Большинство современных приложений используют Json Web Token авторизацию.',
        shortDescEn: 'I will develop authorization layer for your application. Majority apps use Json Web Token Authorization'
      },
      {
        id: 5,
        showDesc: false,
        ...
        shortDescRu: 'Если Вам требуется создать персональный и сложный дизайн, а затем сверстать макеты из PSD файлов, будет привлечен дизайнер, который поможет решить любую Вашу задачу.',
        shortDescEn: 'If you need to create especial and difficult design, I will delegate this task to designer, which can resolve any your tasks.'
      },
      {
        id: 6,
        showDesc: false,
    ...
    typeAdditonalServices: [
      ...
      },
      {
        ...
        shortDescRu: 'Регистрация и настройка Shared-хостинга для вашего проекта.',
