<template>
  <v-form lazy-validation v-model="form.valid" ref="form">
    <v-container fluid v-if="partner">
      <v-row>
        <v-col cols="12">
          <h1>Редактирование партнера для слайдера</h1>
        </v-col>
      </v-row>
      <v-row>
...
export default {
  ...
  computed: {
    ...mapGetters(['getPartnerById']),
    partner() {
      return this.getPartnerById(this.id)
    }
  },
  methods: {
    ...mapActions(['updatePartner', 'getAllPartners', 'uploadImage']),
    upload(event) {
