          ...
          <v-container>
            ...
            <v-row>
              <v-col sm="12" cols="12">
                ...
                <v-text-field v-model="contact" :rules="contactRules" :counter="50" :label="language === 'en' ? labelEn : labelRu" required></v-text-field>
              </v-col>
              <!-- <v-col sm="12" cols="12">
                <v-text-field v-model="phone" :rules="textField"  label="Телефон" required></v-text-field>
              </v-col> -->
              <v-col sm="12" class="text-center" cols="12">
                <v-btn :disabled="!valid" :loading="loading" x-large tile color="primary" @click="submit" class="w100">
        ...
        this.createCallback({ contact: this.contact, result: this.result })
          .then(() => {
            ...
            this.$modal.show(Alert,
              {
                type: 'success',
                message: this.language === 'en' ? 'Request sent successfully!' : 'Запрос успешно отправлен!',
                message2: this.language === 'en' ? 'If you had typed correct contacts, I will contact you soon' : 'Если Вы правильно указали свои контакты, я свяжусь с Вами в ближайшее время.'
              },
              settings)
          ...
          .catch(x => {
            ...
            this.$modal.show(Alert,
              {
                type: 'error',
                message: this.language === 'en' ? 'Request error!' : 'Ошибка при отправке запроса!',
                message2: x
              },
              settings)
            this.$refs.form.reset()
