...
export default {
  ...
  methods: {
    ...mapActions([
      ...
      'deleteNumberSizePromise',
      'updateNumberSizePromise',
      'createNumberSizePromise'
    ]),
    addRow() {
      this.rows.push({
        id: 0,
    ...
    save(item) {
      if (item.name && item.size && item.price) {
        if (item.id === 0) {
          this.createNumberSizePromise(item).then(x => {
            item.id = x.id
            item.edit = false
          })
        } else {
          this.updateNumberSizePromise(item)
            .then(x => {
              item.edit = false
            })
    ...
    confirmDelete() {
      if (this.confirmDialogResult === true) {
        ...
        if (id) {
          this.deleteNumberSizePromise(this.removedItem).then(() => {})
        }
